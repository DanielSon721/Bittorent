...
class BitTorrentClient:
    def __init__(self, torrent, peer_id: bytes, listen_port: int, output_path: Optional[str] = None, max_peers: int = 20):
        ...
        self.peer_id = peer_id  # unique peer identifier
        self.listen_port = listen_port  # port to accept inbound peers
        self.output_path = output_path or torrent.name  # output file path — defaults to torrent.name

        # tracker client wrapper
        ...
        self.piece_manager = PieceManager(torrent)  # handles internal state for pieces
        self.disk = DiskIO(self.output_path, torrent.length)  # handles writing pieces to disk
        self.peers: List[PeerConnection] = []  # list of active PeerConnection objects

        # control flags and threading structures
        self._stop = False
        ...
        # stats for tracker and monitoring
        self.downloaded = 0
        self.uploaded = 0
    ...
    # --- INBOUND PEER LISTENER (accept incoming peer connections) ---
    ...
    # --- PEER CONNECT: TCP CONNECTION AND HANDSHAKE ---
            ...
            while (
                ...
                if not peer.peer_choking:
                    ...
                    if piece_index is not None:
                        ...
                        # request many blocks IN PARALLEL (boosts speed massively)
                        while offset < total_len and len(request_queue) < MAX_PENDING:
                            if not self.piece_manager.has_block(piece_index, offset):
    ...
    def _select_piece(self, peer: PeerConnection) -> Optional[int]:
        for i in range(self.torrent.num_pieces()):
            if (
                ...
                self.piece_manager.mark_piece_in_progress(i)
                return i
        return None



    # --- CALCULATE LENGTH OF PIECE ---
